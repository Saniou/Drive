...
const DEFAULT_LANGUAGE =
    process.env.NEXT_PUBLIC_MAPBOX_LANGUAGE?.trim() ??
    process.env.MAPBOX_LANGUAGE?.trim() ??
    "en";
const DEFAULT_LIMIT =
    process.env.NEXT_PUBLIC_MAPBOX_SUGGEST_LIMIT?.trim() ??
    process.env.MAPBOX_SUGGEST_LIMIT?.trim() ??
    "10";
const DEFAULT_COUNTRY =
    process.env.NEXT_PUBLIC_MAPBOX_COUNTRY?.trim() ??
    process.env.MAPBOX_COUNTRY?.trim() ??
    "US";
...
export async function GET(request: Request) {
    try {
        ...
        if (!params.has("country") && DEFAULT_COUNTRY) {
